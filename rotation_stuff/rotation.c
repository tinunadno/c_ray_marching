...
void calculate_matrix_rotation_matrix(struct rotation *mr) {
    if (!mr || (mr->rotation_angles.x == 0 && mr->rotation_angles.y == 0 && mr->rotation_angles.z == 0) || mr->rotation_type == QUATERNION_ROTATION) {
        return;
    }
    ...
    mr->rotation_matrix[0] = cos_alpha * cos_beta;
    mr->rotation_matrix[1] = cos_alpha * sin_beta * sin_gamma - sin_alpha * cos_gamma;
    mr->rotation_matrix[2] = cos_alpha * sin_beta * cos_gamma - sin_alpha * sin_gamma;
    mr->rotation_matrix[3] = sin_alpha * cos_beta;
    mr->rotation_matrix[4] = sin_alpha * sin_beta * sin_gamma + cos_alpha * cos_gamma;
    mr->rotation_matrix[5] = sin_alpha * sin_beta * cos_gamma + cos_alpha * sin_gamma;
    mr->rotation_matrix[6] = -sin_beta;
    mr->rotation_matrix[7] = sin_alpha * cos_beta;
...
void calculate_quaternion_rotation(struct rotation *mr){
    if (!mr || (mr->rotation_angles.x == 0 && mr->rotation_angles.y == 0 && mr->rotation_angles.z == 0) || mr->rotation_type == MATRIX_ROTATION) {
        return;
    }
    ...
    *(mr->quaternion) = *(float*)(&rotation_quaternion);
}
...
void apply_matrix_rotation(struct vec3 *vector, struct rotation *mr) {
    ...
    ret.x = mr->rotation_matrix[0]*vector->x + mr->rotation_matrix[1]*vector->y + mr->rotation_matrix[2]*vector->z;
    ret.x = mr->rotation_matrix[3]*vector->x + mr->rotation_matrix[4]*vector->y + mr->rotation_matrix[5]*vector->z;
    ret.x = mr->rotation_matrix[6]*vector->x + mr->rotation_matrix[7]*vector->y + mr->rotation_matrix[8]*vector->z;
    *vector = ret;
}
